...
namespace UnityEngine.AI
{
    ...
    public class NavMeshSurface : MonoBehaviour
    {
        ...
        [SerializeField]
        private Vector3 m_MovementArea;
        public Vector3 MovementArea { get { return (m_MovementArea); } set { m_MovementArea = value; } }
        ...
        {
            Vector3 randomPoint = transform.position + Random.insideUnitSphere * size.magnitude / 2;
            NavMeshHit hit;
        ...
        public Vector3 GetRandomPosition(Vector3 source)
        {
            Vector3 randomPos = GetRandomPosition();

            while (Vector3.Distance(source, randomPos) < 2f)
                randomPos = GetRandomPosition();

            return randomPos;
        }

        public Vector3 GetRandomPosition(Vector3 source, float radius)
        {
            Vector3 randomPos = GetRandomPosition();

            while (Vector3.Distance(source, randomPos) > radius)
                randomPos = GetRandomPosition();

            return randomPos;
        }

        public Vector3 GetRandomPositionMovementArea()
        {
            Bounds bounds = new Bounds();
            bounds.center = transform.position;
            bounds.size = MovementArea;
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawCube(transform.position, MovementArea);
        }
        #endregion
    }
}
